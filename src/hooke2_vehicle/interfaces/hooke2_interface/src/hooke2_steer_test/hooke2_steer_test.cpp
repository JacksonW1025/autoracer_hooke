...
#include "hooke2_steer_test/hooke2_steer_test.hpp"
...
Hooke2SteerTest::Hooke2SteerTest()
: Node("hooke2_steer_test"),
  vehicle_info_(vehicle_info_util::VehicleInfoUtil(*this).getVehicleInfo())
{
  ...
  engage_cmd_sub_ = create_subscription<autoware_auto_vehicle_msgs::msg::Engage>(
    "/vehicle/engage", rclcpp::QoS{1}, std::bind(&Hooke2SteerTest::callbackEngage, this, _1));
  // From hooke2
  ...
  hooke2_feedbacks_sync_->registerCallback(std::bind(
    &Hooke2SteerTest::callbackHooke2Rpt, this, std::placeholders::_1, std::placeholders::_2,
  ...
  // To hooke2
  accel_cmd_pub_ =
    create_publisher<hooke2_msgs::msg::SystemCmdFloat>("/hooke2/accel_cmd", rclcpp::QoS{1});
  brake_cmd_pub_ =
  ...
  // Timer
  auto timer_callback = std::bind(&Hooke2SteerTest::publishCommands, this);
...
void Hooke2SteerTest::callbackEngage(
...
void Hooke2SteerTest::callbackHooke2Rpt(
  ...
  const hooke2_msgs::msg::GlobalRpt::ConstSharedPtr global_rpt)
{
  RCLCPP_ERROR_STREAM_THROTTLE(
    get_logger(), *get_clock(), 5000.0, "hooke2 Steer Test Now.");  // for safety
  ...
  RCLCPP_DEBUG(
    get_logger(),
    "[hooke2] enabled: is_hooke2_enabled_ %d, steer %d, accel %d, brake %d, shift %d, global %d",
    is_hooke2_enabled_, steer_wheel_rpt_ptr_->enabled, accel_rpt_ptr_->enabled,
...
void Hooke2SteerTest::publishCommands()
{
  /* guard */
  if (!is_hooke2_rpt_received_) {
    RCLCPP_INFO_THROTTLE(
      get_logger(), *get_clock(), 2000.0, "[hooke2]  hooke2_msgs = %d", is_hooke2_rpt_received_);
    return;
  }
  ...
  if (!prev_override_ && global_rpt_ptr_->override_active) {
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 1000.0, "Hooke2 is overridden, enable flag is back to false");
    engage_cmd_ = false;
  }
  ...
  if (report_timeouted) {
    RCLCPP_WARN_THROTTLE(
      ...
      "Hooke2 report is timeouted, enable flag is back to false");
    engage_cmd_ = false;
  }
  ...
  if (global_rpt_ptr_->hooke2_sys_fault_active) {
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 1000.0, "Hooke2 fault is active, enable flag is back to false");
    engage_cmd_ = false;
  }

  RCLCPP_DEBUG(
    get_logger(),
    "[hooke2] is_hooke2_enabled_ = %d, is_clear_override_needed_ = %d, clear_override = %d",
    is_hooke2_enabled_, is_clear_override_needed_, clear_override);

  /* publish accel cmd */
  {
    hooke2_msgs::msg::SystemCmdFloat accel_cmd;
  ...
  /* publish shift cmd */
  {
    ...
    shift_cmd.command = hooke2_msgs::msg::SystemCmdInt::SHIFT_FORWARD;  // always drive shift
  ...
  turn_cmd.command = hooke2_msgs::msg::SystemCmdInt::TURN_HAZARDS;  // for safety
  turn_cmd_pub_->publish(turn_cmd);
}

double Hooke2SteerTest::testSteerCommand()
{
...
double Hooke2SteerTest::sineWave()
{
...
double Hooke2SteerTest::increasedSineWave()
{
  /*
...
double Hooke2SteerTest::stepWithTwoValue()
{
  static rclcpp::Time ts = now();
  if (!engage_cmd_ || !checkDriveShift()) {
    ts = now();
...
double Hooke2SteerTest::stepWithThreeValue()
{
  static rclcpp::Time ts = now();
  if (!engage_cmd_ || !checkDriveShift()) {
    ts = now();
...
double Hooke2SteerTest::increasedStepWithTwoValue()
{
  static rclcpp::Time ts = now();
  if (!engage_cmd_ || !checkDriveShift()) {
    ts = now();
...
bool Hooke2SteerTest::checkDriveShift()
{
  /* check shift  */
  if (shift_rpt_ptr_->output != hooke2_msgs::msg::SystemCmdInt::SHIFT_FORWARD) {  // need shift
                                                                                   // change.
...
double Hooke2SteerTest::getAccel()
{
  if (!checkDriveShift()) {
    return 0.0;
  }
...
double Hooke2SteerTest::getBrake()
{
  if (!checkDriveShift()) {
...
double Hooke2SteerTest::calculateVehicleVelocity(
...
double Hooke2SteerTest::calculateVariableGearRatio(const double vel, const double steer_wheel)
{
  return std::max(
