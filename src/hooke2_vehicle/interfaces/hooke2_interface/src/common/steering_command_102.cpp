...
void Steeringcommand102::Reset() {
  ...
  steer_en_ctrl_ = SteeringCmd102::STEER_EN_CTRL_DISABLE;
...
Steeringcommand102* Steeringcommand102::set_steer_en_ctrl(
    uint8_t steer_en_ctrl) {
  steer_en_ctrl_ = steer_en_ctrl;
  return this;
}

// config detail: {'name': 'Steer_EN_CTRL', 'uint8': {0: 'STEER_EN_CTRL_DISABLE',
...
// '[-440|440]', 'bit': 31, 'type': 'int', 'order': 'motorola', 'physical_unit':
// 'deg'}
void Steeringcommand102::set_p_steer_angle_target(uint8_t* data,
                                                  int steer_angle_target) {
  steer_angle_target =
      ProtocolData::BoundedValue(-440, 440, steer_angle_target);
