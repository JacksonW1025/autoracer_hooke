...
def test_course_index_uses_fixed_course_offsets_for_boundary_margin():
    samples = [
        CourseSample(float(i), float(i), 0.0, 0.0, 0.0, 0.0, 1.8, 1.8, 5.0, 0.0)
        for i in range(20)
    ]
    index = CourseIndex(samples, vehicle_width_m=1.8, vehicle_length_m=4.0)
    match = index.match(4.0, 0.2, 0.0)
    assert match is not None
    assert match.cross_track_m == pytest.approx(0.2)
    assert match.boundary_margin_m == pytest.approx(0.7)


def test_gate_safe_profile_has_complete_equal_length_filter_arrays():
    path = PACKAGE / "config/urbanroad_sim/vehicle_cmd_gate.safe.param.yaml"
