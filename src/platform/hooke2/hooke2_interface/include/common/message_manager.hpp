 ...
 */
#ifndef COMMON__MESSAGE_MANAGER_HPP_
#define COMMON__MESSAGE_MANAGER_HPP_
...
using rclcpp::Time;
using hooke2::common::ProtocolData;
using wd_byte::Byte;
...
class MessageManager {
 public:
   ...
   */
  ErrorCode GetSensorData(SensorType *const sensor_data);
...
ProtocolData<SensorType>
  ...
  if (protocol_data_map_.find(message_id) == protocol_data_map_.end()) {
    RCLCPP_ERROR(rclcpp::get_logger("hooke2_interface.hooke2_message_manager"), 
    "Unable to get protocol data because of invalid message_id: %s" , Byte::byte_to_hex(message_id).c_str());
    return nullptr;
  }
...
void MessageManager<SensorType>::Parse(const uint32_t message_id,
  ...
  if (it != check_ids_.end()) {
    const int64_t time = rclcpp::Clock().now().nanoseconds() / 1e3;
...
void MessageManager<SensorType>::ResetSendMessages() {
  for (auto &protocol_data : send_protocol_data_) {
    if (protocol_data == nullptr) {
      RCLCPP_ERROR(rclcpp::get_logger("hooke2_interface.hooke2_message_manager"), "Invalid protocol data.");
    } else {
      protocol_data->Reset();
    }
  }
...
#endif  // COMMON__MESSAGE_MANAGER_HPP_