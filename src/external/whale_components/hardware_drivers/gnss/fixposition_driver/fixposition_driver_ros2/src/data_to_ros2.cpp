...
// void PublishAutowareOrientationData(const OdometryData& data,
//                 rclcpp::Publisher<autoware_sensing_msgs::msg::GnssInsOrientationStamped>::SharedPtr aw_pub) {
//     if (aw_pub->get_subscription_count() > 0) {
//         // Create message
//         autoware_sensing_msgs::msg::GnssInsOrientationStamped aw_msg;

//         // Populate message
//         if (data.stamp.sec_ == 0.0 && data.stamp.nsec_ == 0) {
//             aw_msg.header.stamp = rclcpp::Clock().now();
//         } else {
//             aw_msg.header.stamp = ros2::utils::ConvTime(data.stamp);;
//         }
//         aw_msg.header.frame_id = "map";  // 使用map坐标系

//         // 1. 将ECEF坐标系的orientation转换为ENU坐标系
//         // 首先获取当前位置的经纬度
//         const Eigen::Vector3d llh_pos = fpsdk::common::trafo::TfWgs84LlhEcef(data.pose.position);
//         const double lat = llh_pos(0);
//         const double lon = llh_pos(1);
        
//         // 2. 计算ECEF到ENU的旋转矩阵
//         const Eigen::Matrix3d rot_ecef_enu = fpsdk::common::trafo::RotEnuEcef(lat, lon);
        
//         // 3. 将ECEF坐标系的orientation转换为ENU坐标系
//         const Eigen::Matrix3d rot_enu_body = rot_ecef_enu * data.pose.orientation.toRotationMatrix();
        
//         // 4. 将ENU坐标系的orientation转换为localUTM坐标系
//         // 计算UTM区域和网格收敛角
//         const int utm_zone = static_cast<int>((lon * 180.0 / M_PI + 180.0) / 6.0) + 1;
//         const double central_meridian = (utm_zone - 1) * 6.0 - 180.0 + 3.0; // 度
//         const double central_meridian_rad = central_meridian * M_PI / 180.0;
//         const double grid_convergence = (lon - central_meridian_rad) * sin(lat);
        
//         // 5. 创建UTM转换的旋转矩阵（绕Z轴旋转网格收敛角）
//         const Eigen::AngleAxisd utm_rotation(-grid_convergence, Eigen::Vector3d::UnitZ());
//         const Eigen::Matrix3d rot_utm_enu = utm_rotation.toRotationMatrix();
        
//         // 6. 将ENU坐标系转换为localUTM坐标系
//         const Eigen::Matrix3d rot_utm_body = rot_utm_enu * rot_enu_body;
        
//         // 7. 转换为四元数
//         const Eigen::Quaterniond utm_quat(rot_utm_body);
//         // 8. 设置orientation
//         aw_msg.orientation.orientation.x = utm_quat.x();
//         aw_msg.orientation.orientation.y = utm_quat.y();
//         aw_msg.orientation.orientation.z = utm_quat.z();
//         aw_msg.orientation.orientation.w = utm_quat.w();
        
//         // 9. 设置RMSE（需要转换协方差矩阵）
//         const Eigen::Matrix3d rot_cov_enu = rot_ecef_enu * data.pose.cov.block<3,3>(3,3) * rot_ecef_enu.transpose();
//         const Eigen::Matrix3d rot_cov_utm = rot_utm_enu * rot_cov_enu * rot_utm_enu.transpose();
        
//         aw_msg.orientation.rmse_rotation_x = sqrt(rot_cov_utm(0,0));
//         aw_msg.orientation.rmse_rotation_y = sqrt(rot_cov_utm(1,1));
//         aw_msg.orientation.rmse_rotation_z = sqrt(rot_cov_utm(2,2));

//         // Publish message
//         aw_pub->publish(aw_msg);
//     }
// }
void PublishAutowareOrientationData(const OdometryData& data,
                rclcpp::Publisher<autoware_sensing_msgs::msg::GnssInsOrientationStamped>::SharedPtr aw_pub) {
    if (aw_pub->get_subscription_count() > 0) {
        ...
        aw_msg.header.frame_id = "FP_ECEF";

        // 1. 获取当前位置的经纬度（弧度）
        // const Eigen::Vector3d llh_pos = fpsdk::common::trafo::TfWgs84LlhEcef(data.pose.position);
        // const double lat = llh_pos(0);  // 弧度
        // const double lon = llh_pos(1);  // 弧度
        
        // // 2. 计算ECEF到ENU的旋转矩阵（直接使用ECEF位置更准确）
        // const Eigen::Matrix3d rot_ecef_enu = fpsdk::common::trafo::RotEnuEcef(data.pose.position);
        
        // // 3. 将ECEF坐标系的orientation转换为ENU坐标系
        // // data.pose.orientation 是 body 相对于 ECEF 的旋转
        // const Eigen::Matrix3d rot_enu_body = rot_ecef_enu * data.pose.orientation.toRotationMatrix();
        
        // // 4. 将ENU坐标系的orientation转换为localUTM坐标系
        // // 计算UTM区域和网格收敛角
        // const double lon_deg = lon * 180.0 / M_PI;
        // const int utm_zone = static_cast<int>((lon_deg + 180.0) / 6.0) + 1;
        // const double central_meridian_deg = (utm_zone - 1) * 6.0 - 180.0 + 3.0;
        // const double central_meridian_rad = central_meridian_deg * M_PI / 180.0;
        // // 网格收敛角：更精确的公式（对于大多数应用，sin(lat)近似已经足够）
        // const double grid_convergence = (lon - central_meridian_rad) * sin(lat);
        
        // // 5. 创建UTM转换的旋转矩阵（绕Z轴旋转网格收敛角）
        // // 注意：旋转方向需要根据Autoware的localUTM定义确认
        // const Eigen::AngleAxisd utm_rotation(-grid_convergence, Eigen::Vector3d::UnitZ());
        // const Eigen::Matrix3d rot_utm_enu = utm_rotation.toRotationMatrix();
        ...
        // // 7. 转换为四元数并归一化
        // Eigen::Quaterniond utm_quat(rot_utm_body);
        // utm_quat.normalize();  // 确保归一化
        
        // 8. 设置orientation
        aw_msg.orientation.orientation = tf2::toMsg(data.pose.orientation);
        // aw_msg.orientation.orientation.y = utm_quat.y();
        // aw_msg.orientation.orientation.z = utm_quat.z();
        // aw_msg.orientation.orientation.w = utm_quat.w();
        ...
        // 9. 设置RMSE（需要转换协方差矩阵）
        // 获取旋转部分的协方差矩阵（右下角3x3）
        // const Eigen::Matrix3d rot_cov_ecef = data.pose.cov.block<3,3>(3,3);
        // // 转换到ENU坐标系
        // const Eigen::Matrix3d rot_cov_enu = rot_ecef_enu * rot_cov_ecef * rot_ecef_enu.transpose();
        // // 转换到UTM坐标系
        // const Eigen::Matrix3d rot_cov_utm = rot_utm_enu * rot_cov_enu * rot_utm_enu.transpose();
        // Eigen::Map<Eigen::Matrix<double, 6, 6>> cov_map = Eigen::Map<Eigen::Matrix<double, 6, 6>>(msg.covariance.data());
        // cov_map = data.cov; 
        // RMSE是标准差，取对角元素的平方根
        aw_msg.orientation.rmse_rotation_x = 0.01;
        aw_msg.orientation.rmse_rotation_y = 0.01;
        aw_msg.orientation.rmse_rotation_z = 0.09;
        // Eigen::Map<Eigen::Matrix<double, 6, 6>> cov_map = Eigen::Map<Eigen::Matrix<double, 6, 6>>(msg.covariance.data());
        // cov_map = data.cov;
        // aw_msg.orientation.rmse
