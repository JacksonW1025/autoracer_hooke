...

{
    ...
    // @liangweiyu 20251103
    if (params_.qos_type_ == "sensor_short") {
        qos_settings_ = RMW_QOS_POLICY_RELIABILITY_RELIABLE;
    }
    // - Long-queue sensor-type QoS
    else if (params_.qos_type_ == "sensor_long") {
        qos_settings_ = RMW_QOS_POLICY_RELIABILITY_RELIABLE;
...
bool FixpositionDriverNode::StartNode() {
    ...
    if (params_.MessageEnabled(fpa::FpaOdometryPayload::MSG_NAME)) {
        ...
        _PUB(odometry_llh_pub_, sensor_msgs::msg::NavSatFix, output_ns + "/fix", qos_settings_);
        _PUB(poiimu_pub_, sensor_msgs::msg::Imu, output_ns + "/rawimu", qos_settings_);
        _PUB(autoware_orientation_pub_, autoware_sensing_msgs::msg::GnssInsOrientationStamped, output_ns + "/autoware_orientation", qos_settings_);
        driver_.AddFpaObserver(fpa::FpaOdometryPayload::MSG_NAME, [this](const fpa::FpaPayload& payload) {
            ...
            // OdomToYprMsg(odometry_data, autoware_orientation_pub_);
            // PublishAutowareOrientationData(odometry_data, autoware_orientation_pub_);
    ...
    if (params_.MessageEnabled(fpa::FpaRawimuPayload::MSG_NAME)) {
        _PUB(rawimu_pub_, fpmsgs::FpaImu, output_ns + "/fparawimu", qos_settings_);
...
void FixpositionDriverNode::StopNode() {
    ...
    autoware_orientation_pub_.reset();
}
