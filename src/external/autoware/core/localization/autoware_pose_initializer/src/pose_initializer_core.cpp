...
#include "direct_pose_validation.hpp"
  ...
  group_srv_(create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive))
{
  ...
  direct_pose_timeout_sec_ = declare_parameter<double>("direct_pose_timeout_sec", 0.5);
  ...
  const geometry_msgs::msg::Pose initial_pose, bool need_spin)
{
  ...
  set_user_defined_initial_pose(pose, need_spin);
}

void PoseInitializer::set_user_defined_initial_pose(
  const PoseWithCovarianceStamped & initial_pose_msg, bool need_spin)
{
  try {
    change_state(State::Message::INITIALIZING);
    change_node_trigger(false, need_spin);

    PoseWithCovarianceStamped pose = initial_pose_msg;
  ...
  const Initialize::Service::Response::SharedPtr res)
{
  try {
    ...
    if (req->method == Initialize::Service::Request::AUTO) {
      ...
      auto pose = req->pose_with_covariance.front();
      if (!direct_pose_stamp_is_fresh(
            rclcpp::Time(pose.header.stamp), this->now(), direct_pose_timeout_sec_)) {
        std::stringstream message;
        message << "DIRECT initial pose is stale. pose_stamp="
                << rclcpp::Time(pose.header.stamp).seconds() << ", now=" << this->now().seconds()
                << ", timeout=" << direct_pose_timeout_sec_;
