  ...
  const PointCloud2ConstPtr & input, const rclcpp::Logger & logger)
{
  ...
  if (
    ...
    (input->fields[intensity_index_].datatype != sensor_msgs::msg::PointField::UINT8 &&
     input->fields[intensity_index_].datatype != sensor_msgs::msg::PointField::FLOAT32)) {
    RCLCPP_ERROR(
      logger,
  ...
  if (intensity_index_ != -1) {
    ...
    intensity_datatype_ = input->fields[intensity_index_].datatype;
  } else {
    intensity_offset_ = -1;
    intensity_datatype_ = -1;
  }
...
{
  float intensity = 0.0;
  if (intensity_index_ >= 0 && intensity_datatype_ == sensor_msgs::msg::PointField::UINT8) {
    intensity = static_cast<float>(
      *reinterpret_cast<const uint8_t *>(&input->data[global_offset + intensity_offset_]));
  } else if (
    intensity_index_ >= 0 && intensity_datatype_ == sensor_msgs::msg::PointField::FLOAT32) {
